...
export function DockerHero({ terminal }: DockerHeroProps) {
  return (
    <GuideHero
      ...
      description={
        <p>
          把组件站点与 MCP 服务一起打包，部署到你的机器或团队服务器。打开浏览器挑选灵感，让 AI
          随时取用。
        </p>
      }
      actions={
        <>
          ...
          <a href="#install" className="button-secondary text-xs">
            查看部署步骤
      ...
      stats={stats}
      links={[
        { id: 'install', label: '获取镜像' },
        { id: 'services', label: '容器服务' },
        { id: 'operations', label: '日常维护' },
      ]}
    >
      <div className="relative mx-auto max-w-[560px]">
        <div
          aria-hidden
          className="pointer-events-none absolute -inset-10 overflow-hidden rounded-full opacity-40 blur-3xl"
        >
          <AuroraBackground
            colors={['#233b19', '#122b28', '#343e18']}
            speed={22}
            className="h-full"
          />
        </div>
        <div className="relative">
          <div className="mb-4 flex items-center justify-between px-1">
            ...
            <span className="font-mono text-[9px] text-mute/55">ai-ui / latest</span>
          </div>
          ...
          <p className="mt-4 text-right text-[10px] text-mute/60">
            容器启动后，即可通过以上地址访问
          </p>
        </div>
      </div>
