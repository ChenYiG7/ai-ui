...
const resources: ResourceLink[] = [
  ...
  { href: '/api/prompt/magnetic-button', label: '提示词 API' },
];
    ...
    <footer className="relative overflow-hidden border-t border-line bg-[#0c0d0e]">
      <div className="page-shell pb-7 pt-14 md:pt-20">
        <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-[1.3fr_1fr_0.65fr] lg:gap-20">
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-6 inline-flex items-center gap-2 text-xs text-mute transition-colors hover:text-accent"
            >
              在 GitHub 上一起构建
            ...
            <ul className="mt-5 space-y-3.5 text-xs">
              {resources.map((resource) => (
                <li key={resource.href}>
                  <a
                    href={resource.href}
                    className="group inline-flex items-center gap-2 text-mute transition-colors hover:text-accent"
                  >
                    {resource.label}
                    <ArrowUpRight
                      aria-hidden
        ...
        <div className="mt-14 flex items-end justify-between gap-6 border-b border-line pb-7 md:mt-20">
          <span
            aria-hidden
            className="text-[clamp(4rem,13vw,10rem)] font-semibold leading-[0.8] tracking-[-0.085em] text-white/[0.06]"
          >
            ai-ui<span className="text-accent/25">.</span>
          </span>
          <Link
            href="/#gallery"
            aria-label="浏览组件词典"
            className="group mb-1 flex items-center gap-3 text-xs text-mute transition-colors hover:text-ink"
          >
            <span className="hidden sm:inline">把下一份灵感，变成界面</span>
            <span className="grid size-10 place-items-center rounded-full border border-line transition-colors group-hover:border-accent group-hover:text-accent">
        ...
        <div className="mt-6 flex flex-col gap-3 text-[10px] text-mute sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="flex items-center gap-2">
            ...
            由词典中的组件构建<span className="ml-3 font-mono text-mute/65">{site.volume}</span>
          </span>
        </div>
      </div>
    </footer>
