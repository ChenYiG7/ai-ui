...
export function SiteHeader() {
  ...
  const isActive = (path: string) =>
    path === '/' ? pathname === '/' || pathname.startsWith('/c/') : pathname === path;
    ...
    <header className="sticky top-0 z-40 border-b border-line bg-canvas/85 backdrop-blur-2xl">
      <div className="page-shell flex h-[72px] items-center justify-between gap-6">
        ...
        >
          <span
            aria-hidden
            className="relative grid size-8 place-items-center rounded-lg border border-accent/25 bg-accent/5"
          >
          ...
          <span className="ml-1 hidden border-l border-line pl-3 text-[11px] text-mute xl:inline">
            组件与灵感的词典
          </span>
        </Link>
        <nav aria-label="主导航" className="hidden items-center gap-8 md:flex">
          {links.map((link) => (
            <Link
              ...
              className={cn(
                'relative py-6 text-xs transition-colors hover:text-ink',
                isActive(link.path) ? 'text-ink' : 'text-mute',
              )}
            >
              {link.label}
              {isActive(link.path) ? (
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="hidden items-center gap-1.5 pl-2 text-xs text-mute transition-colors hover:text-ink lg:inline-flex"
          >
      ...
      {menuOpen ? (
        <nav
          ...
          aria-label="移动端导航"
          className="absolute inset-x-0 top-full border-b border-line bg-panel p-5 shadow-2xl md:hidden"
          onKeyDown={(event) => {
            if (event.key === 'Escape') {
              setMenuOpen(false);
        ...
        >
          {links.map((link) => (
            <Link
              ...
              aria-current={isActive(link.path) ? 'page' : undefined}
              className={cn(
                'flex items-center justify-between rounded-lg px-4 py-3.5 text-sm',
                isActive(link.path)
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-2 flex items-center justify-between border-t border-line px-4 pt-5 text-sm text-mute"
          >
            GitHub 源码仓库
            <ArrowUpRight aria-hidden className="size-4" />
          </a>
