...
export function CommandPalette({ entries }: CommandPaletteProps) {
  ...
  const results = useMemo(() => searchEntries(entries, query).slice(0, 8), [entries, query]);
    ...
    >
      <div className="flex shrink-0 items-center gap-3 border-b border-line px-5">
        ...
        <input
          ...
          placeholder="寻找组件、动效或一点灵感…"
          className="h-16 min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-mute/60"
        />
        <button
          type="button"
          onClick={close}
          aria-label="关闭搜索"
          className="grid size-7 shrink-0 place-items-center rounded-lg border border-line text-mute transition-colors hover:text-ink"
        >
          ...
          <li key={entry.slug} role="presentation">
            <button
              ...
              className={cn(
                ...
                index === active ? 'bg-accent/[0.07]' : 'hover:bg-white/[0.025]',
              )}
            >
              <span
                className={cn(
                  'w-14 shrink-0 truncate font-mono text-[9px] tracking-wider sm:w-20',
                  index === active ? 'text-accent/75' : 'text-mute/60',
                )}
              >
              ...
              {index === active ? (
                <CornerDownLeft aria-hidden className="size-3.5 shrink-0 text-accent/65" />
              ) : null}
            </button>
          </li>
        ))}
      ...
      {results.length === 0 ? (
        <div role="status" className="px-6 py-12 text-center">
          <SearchX aria-hidden className="mx-auto size-6 text-mute/50" />
          <p className="mt-4 text-sm">没有找到匹配的组件</p>
          <p className="mt-2 text-xs text-mute">试试「文字」「发光」或组件的英文名称。</p>
        </div>
      ) : null}
      <div className="flex shrink-0 items-center justify-between gap-3 border-t border-line bg-black/15 px-5 py-3 text-[9px] text-mute/70">
        <span className="inline-flex items-center gap-1">
          ...
          <ArrowDown aria-hidden className="mr-1 size-3" />
          切换
          <CornerDownLeft aria-hidden className="ml-3 size-3" />
          打开
        </span>
        <span>Esc 关闭</span>
      </div>
