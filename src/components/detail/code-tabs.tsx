          ...
          {tabs.map((tab, index) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'relative shrink-0 px-3 py-4 text-[11px] transition-colors sm:px-4 sm:text-xs',
                  selected ? 'text-accent' : 'text-mute hover:text-ink',
                )}
              >
                {tab.label}
                {selected ? (
                  <span
                    aria-hidden
                    className="absolute inset-x-3 bottom-0 h-0.5 bg-accent sm:inset-x-4"
                  />
                ) : null}
              </button>
            );
