    ...
    <div className="surface-panel overflow-hidden">
      <div className="flex min-h-14 items-center justify-between gap-3 border-b border-line bg-white/[0.015] px-4 sm:px-5">
        <span className="flex items-center gap-2 text-xs text-ink/80">
          <Eye aria-hidden className="size-3.5 text-mute" />
          实时预览
        </span>
        <div className="flex items-center gap-3">
          <div
            aria-label="预览宽度"
            className="hidden items-center rounded-lg border border-line bg-black/20 p-0.5 sm:flex"
          >
            <button
              type="button"
              aria-label="宽屏预览"
              ...
              className={cn(
                ...
                !compact ? 'bg-white/10 text-ink' : 'text-mute hover:text-ink',
              )}
            >
      ...
      <div className="bg-[#070708]">
        <PreviewFrame
          key={round}
          className={cn(
            'mx-auto min-h-[420px] w-full lg:min-h-[500px]',
            compact && 'max-w-[375px] border-x border-line',
          )}
