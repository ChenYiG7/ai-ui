...
interface GalleryNavProps {
  items: NavItem[];
  activeId: CategoryId | 'all' | 'favorites';
  query: string;
  resultCount: number | null;
  onQuery: (query: string) => void;
  onNavigate: (id: CategoryId | 'all' | 'favorites') => void;
}
  ...
  return (
    <>
        ...
        <div className="sticky top-24">
          <div className="mb-5 flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] text-mute/60">
              {String(items[0]?.count ?? 0).padStart(2, '0')} ITEMS
            </span>
          </div>
                    ...
                    >
                      <span
                        ...
                        className={cn(
                          ...
                          active ? 'bg-accent' : 'bg-white/15 group-hover:bg-white/40',
                        )}
                      />
                      ...
                      <span
                        className={cn(
                          'font-mono text-[10px]',
                          active ? 'text-accent/70' : 'text-mute/55',
                        )}
                      >
          ...
          <Link
            href="/mcp"
            className="group mt-8 block rounded-xl border border-line bg-gradient-to-br from-white/[0.035] to-transparent p-4 transition-colors hover:border-accent/25"
          >
            ...
            <span className="mt-3 flex items-center gap-2 text-[10px] text-mute group-hover:text-accent">
              了解接入方式
      ...
      <div className="sticky top-[72px] z-30 -mx-5 mb-6 border-y border-line bg-canvas/95 px-5 py-3 backdrop-blur-xl sm:-mx-8 sm:px-8 lg:hidden">
        <SearchField query={query} onQuery={onQuery} />
        {resultCount !== null ? (
          <p role="status" className="mt-3 font-mono text-[10px] text-mute">
            {resultCount} 个匹配的组件
          </p>
        ) : (
          <nav
          ...
          >
            {items.map((item) => (
              <button
                ...
                className={cn(
                  'inline-flex min-h-8 shrink-0 items-center gap-2 rounded-full border px-3 text-[11px] transition-colors',
                  item.id === activeId
