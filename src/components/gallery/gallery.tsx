...
type Selection = CategoryId | 'all' | 'favorites';
...
/**
 * 首页画廊编排层：目录选中式渲染 + 关键词搜索 + 本地收藏视图。
 * 选中态完全由 ?category= 派生（router.replace 驱动）：选中分类只渲染该节，
 * favorites 为收藏夹、all 为分节总览；被动滚动不回写 URL。
 */
export function Gallery({ items }: GalleryProps) {
  ...
  const selected: Selection =
    raw === 'favorites' ? 'favorites' : isCategory(raw) ? raw : 'all';
  ...
  const results = useMemo(() => {
    ...
    const kept = new Set(searchEntries(summaries, query).map((s) => s.slug));
    return items.filter((i) => kept.has(i.summary.slug));
  }, [items, query]);

  // 展示按 registry 编号序，与存储中的收藏先后无关
  const favoriteItems = useMemo(
    () => items.filter((i) => favorites.includes(i.summary.slug)),
    [items, favorites],
  );
  ...
  const navItems: NavItem[] = [
    ...
    { id: 'favorites', label: '我的收藏', code: 'FAV', count: favorites.length },
  ...
  return (
    ...
    <div className="lg:grid lg:grid-cols-[12rem_minmax(0,1fr)] lg:gap-9 xl:gap-12">
      ...
      <div className="min-w-0">
        {results ? (
          <p role="status" className="mb-5 hidden text-xs text-mute lg:block">
            找到 <span className="font-mono text-accent">{results.length}</span> 个匹配的组件
          </p>
        ) : null}
        {results ? (
          results.length > 0 ? (
            ...
            <div className="surface-panel flex min-h-72 flex-col items-center justify-center px-6 py-16 text-center">
              ...
              <p className="mt-2 text-xs leading-6 text-mute">
                试试组件名称、动效描述，或「按钮」「发光」这样的关键词。
              </p>
              <button
                type="button"
                onClick={() => setQuery('')}
                className="button-secondary mt-6 h-9 text-xs"
              >
                清空搜索，继续探索
              </button>
            </div>
          ...
          favoriteItems.length > 0 ? (
            <>
              <p role="status" className="mb-5 hidden text-xs text-mute lg:block">
                已收藏 <span className="font-mono text-accent">{favoriteItems.length}</span>{' '}
                个组件，仅保存在本机浏览器
              </p>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="surface-panel flex min-h-72 flex-col items-center justify-center px-6 py-16 text-center">
              <HeartOff aria-hidden className="size-7 text-mute/60" />
              <h3 className="mt-5 text-base font-medium">收藏夹还是空的</h3>
              <p className="mt-2 max-w-xs text-xs leading-6 text-mute">
                在组件卡片或详情页点亮 ♥，把喜欢的效果留在这里。收藏只保存在你的浏览器里。
              </p>
            </div>
          )
        ) : selectedSection ? (
