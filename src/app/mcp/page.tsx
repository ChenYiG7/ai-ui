...
const ADD_CMD = 'claude mcp add ai-ui -- pnpm mcp';
...
const tools = [
  {
    ...
    output: '| 10 | shimmer-button | 流光按钮 | buttons | … |',
  },
  {
    icon: FileText,
  ...
  },
  {
    ...
    title: '取安装清单',
    body: '按 slug 返回机器可读的 JSON 清单：目标文件路径、完整源码、依赖列表与集成示例。代理解析后直接写入项目，无需人工搬运。',
    params: ['slug'],
    output: '{ "file": "src/components/ui/…", "deps": […] }',
  },
];
...
export const metadata: Metadata = {
  title: 'MCP 服务器',
  description:
    'AI-UI 内置 MCP 服务器：list_components、get_component_prompt、get_component_manifest 三个工具，把整本组件词典直接接进 Claude Code、Cursor 等 AI 客户端。',
};

export default async function McpPage() {
  const [stdioHtml, httpHtml, sourceHtml] = await Promise.all([
    highlightCode(STDIO_SNIPPET, 'bash'),
    highlightCode(HTTP_SNIPPET, 'bash'),
    highlightCode(SOURCE_SNIPPET, 'bash'),
  ]);
  ...
  return (
    <>
      ...
      <GuideSection
        ...
        label="三个工具"
        title="目录、内页与清单，刚好够用。"
        description="先找到适合的组件，再取回提示词或安装清单。几次调用，把检索、设计与实现接在一起。"
      >
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {tools.map((tool, index) => (
            <SpotlightCard
              key={tool.name}
              color="rgba(215,255,60,0.08)"
              className={
                'bg-panel p-6 sm:p-8' +
                (index === tools.length - 1 ? ' md:col-span-2 lg:col-span-1' : '')
              }
            >
              <div className="flex items-center justify-between gap-3">
                <span className="grid size-11 place-items-center rounded-xl border border-accent/15 bg-accent/[0.04]">
                  <tool.icon aria-hidden className="size-4.5 text-accent/80" />
                </span>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-line pt-5">
                ...
                {tool.params.map((param) => (
                  <span
                    key={param}
                    className="rounded-md border border-line bg-white/[0.025] px-2 py-1 font-mono text-[10px] text-mute"
                  >
                    {param}
                  </span>
                ))}
              </div>
              <p className="mt-4 overflow-x-auto rounded-lg border border-line bg-black/25 px-3 py-3 font-mono text-[10px] whitespace-nowrap text-mute/70">
                {tool.output}
              </p>
            </SpotlightCard>
          ))}
      ...
      >
        <div className="grid items-center gap-8 lg:grid-cols-[0.7fr_1.3fr] lg:gap-16">
          <div className="max-w-sm">
            ...
            <h3 className="mt-4 text-2xl font-medium leading-relaxed tracking-tight">
              留在熟悉的编辑器里，
              <br />
              让创作保持连贯。
            </h3>
            <p className="mt-5 text-sm leading-7 text-mute">
              从挑选效果，到读取设计要点和完整源码，AI
              会完成中间的检索。你可以继续调整颜色、文案与布局，让组件成为作品的一部分。
            </p>
            <p className="mt-7 border-l border-accent/30 pl-4 text-xs leading-6 text-mute">
              支持 Claude Code、Cursor、Windsurf 等 MCP 客户端。右侧为组件检索与接入的对话演示。
            </p>
          </div>
