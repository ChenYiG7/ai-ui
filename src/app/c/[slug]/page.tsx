...
import { type CodeTab, CodeTabs } from '@/components/detail/code-tabs';
import { KeyboardPager } from '@/components/detail/keyboard-pager';
...
export default async function ComponentPage({ params }: Params) {
  ...
  const [promptHtml, sourceHtml, usageHtml] = await Promise.all([
    highlightCode(prompt, 'markdown'),
    highlightCode(source, 'tsx'),
    highlightCode(entry.usage, 'tsx'),
  ]);
  ...
  return (
    <article className="page-shell pb-20 pt-7 md:pb-28 md:pt-9">
      <nav
        aria-label="面包屑导航"
        className="flex min-w-0 items-center gap-2.5 text-[11px] text-mute"
      >
        <Link
          href="/#gallery"
          className="inline-flex shrink-0 items-center gap-2 transition-colors hover:text-ink"
        >
      ...
      <header className="mb-9 mt-10 grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end md:mb-11 md:mt-12">
        <div className="min-w-0">
          <p className="eyebrow flex items-center gap-3">
            <span className="text-accent">No.{formatIndex(index)}</span>
            <span aria-hidden className="h-px w-5 bg-white/20" />
            {entry.name}
          </p>
          <h1 className="mt-4 text-4xl font-medium leading-tight tracking-[-0.045em] sm:text-5xl lg:text-6xl">
            {entry.title}
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 lg:flex-col lg:items-end">
          <div className="flex items-center gap-2.5">
            ...
            <FavoriteButton slug={entry.slug} title={entry.title} withLabel />
          </div>
        ...
        <aside className="order-2 min-w-0 lg:order-1">
          <div className="surface-panel p-5 sm:p-6 lg:sticky lg:top-24">
            ...
            <ol className="mt-6 space-y-5">
              {entry.designNotes.map((note, noteIndex) => (
                <li key={note} className="flex gap-3.5 text-xs leading-6">
                  <span className="mt-0.5 font-mono text-[10px] text-accent/70">
            ...
            <div className="mt-6 border-t border-line pt-5">
              <p className="flex items-center gap-2 text-[10px] text-mute">
                <Package aria-hidden className="size-3" />
                {entry.deps.length ? '依赖 ' + entry.deps.join(' · ') : '无需额外依赖'}
              </p>
              <ul aria-label="组件标签" className="mt-4 flex flex-wrap gap-2">
                {entry.tags.map((tag) => (
                  <li
                    key={tag}
                    className="rounded-md border border-line bg-white/[0.02] px-2 py-1 text-[10px] text-mute"
                  >
                    {tag}
                  </li>
                ))}
      ...
      <nav aria-label="组件翻页" className="mt-14 border-t border-line pt-7 md:mt-20">
        <div className="mb-5 flex items-center justify-between gap-4">
          ...
          <span className="flex items-center gap-4 font-mono text-[10px] text-mute">
            <span aria-hidden className="hidden items-center gap-1.5 text-mute/60 sm:inline-flex">
              <kbd className="rounded border border-line bg-white/[0.03] px-1.5 py-0.5 text-[9px] leading-none">
                ←
              </kbd>
              <kbd className="rounded border border-line bg-white/[0.03] px-1.5 py-0.5 text-[9px] leading-none">
                →
            ...
            {formatIndex(index)} / {registry.length}
          </span>
        </div>
        <KeyboardPager
          prevHref={prev ? '/c/' + prev.slug : undefined}
          nextHref={next ? '/c/' + next.slug : undefined}
        />
        <div className="grid gap-4 md:grid-cols-2">
          {prev ? (
            <Link
              href={'/c/' + prev.slug}
              className="surface-panel group flex items-center gap-5 p-5 transition-colors hover:border-accent/25 sm:p-6"
            >
              <span className="grid size-9 place-items-center rounded-full border border-line text-mute transition-colors group-hover:border-accent/25 group-hover:text-accent">
                <ArrowLeft
                  aria-hidden
              ...
              <span className="min-w-0">
                ...
                <span className="mt-1 block truncate font-mono text-[10px] text-mute/60">
                  {prev.name}
                </span>
              </span>
            </Link>
          ...
          {next ? (
            <Link
              href={'/c/' + next.slug}
              className="surface-panel group flex items-center justify-end gap-5 p-5 text-right transition-colors hover:border-accent/25 sm:p-6"
            >
              <span className="min-w-0">
