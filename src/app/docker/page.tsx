...
const RUN_CMD = `docker run -d --name ai-ui \\
  -p 3000:3000 -p 8787:8787 \\
  ${site.dockerImage}`;

const PULL_SNIPPET = `# 方式一：Docker Hub 直接拉取（镜像发布后可用）
docker run -d --name ai-ui \\
  -p 3000:3000 -p 8787:8787 \\
  ${site.dockerImage}
...
const OPS_SNIPPET = `docker logs -f ai-ui        # 启动日志里打印两个访问地址
docker restart ai-ui        # 重启容器
docker rm -f ai-ui          # 停止并移除
...
const services = [
  ...
  },
  {
    icon: Bot,
    title: 'MCP · 端口 8787',
    body: 'streamable-http 传输的 MCP 服务器。AI 客户端连上 /mcp，即可调用词典的三个查询工具，接口细节见上一页。',
    meta: 'http://localhost:8787/mcp',
  },
  {
    icon: SlidersHorizontal,
    title: '端口可调 · 日志可查',
    body: 'WEB_PORT / MCP_PORT 环境变量覆盖容器内监听端口；docker logs -f ai-ui 随时查看启动日志与访问地址。',
...
export const metadata: Metadata = {
  title: 'Docker 部署',
  description:
    '一条 docker run 同时启动 AI-UI 站点与 MCP 服务器：nginx 伺服静态页面，8787 端口把词典暴露给 AI 客户端。',
};

export default async function DockerPage() {
  const [runHtml, pullHtml, buildHtml, opsHtml] = await Promise.all([
    highlightCode(RUN_CMD, 'bash'),
    highlightCode(PULL_SNIPPET, 'bash'),
    highlightCode(BUILD_SNIPPET, 'bash'),
    highlightCode(OPS_SNIPPET, 'bash'),
  ]);
  ...
  const tabs: CodeTab[] = [
    {
      ...
      raw: PULL_SNIPPET,
      html: pullHtml,
      hint: '镜像发布到 Docker Hub 后可直接拉取，适合大多数场景；启动日志会打印两个访问地址。',
    },
    {
      id: 'build',
      label: '本地构建',
      raw: BUILD_SNIPPET,
      html: buildHtml,
      hint: '想改词典内容或站点文案时，从源码构建自己的镜像。需 Docker 23+（BuildKit 默认开启）。',
    },
  ];
            ...
            >
              <div className="flex items-center justify-between">
                <span className="grid size-11 place-items-center rounded-xl border border-accent/15 bg-accent/[0.04]">
                  <service.icon aria-hidden className="size-4.5 text-accent/80" />
                </span>
              ...
              <p className="mt-5 break-all rounded-lg border border-line bg-black/20 px-3 py-3 font-mono text-[10px] leading-5 text-mute/75">
                {service.meta}
              </p>
            </SpotlightCard>
          ))}
