  ...
  return (
    <div className={cn('relative isolate overflow-hidden bg-[#080c12]', className)}>
      <div
        ...
        style={{
          background: `radial-gradient(ellipse at 50% 110%, color-mix(in srgb, ${colors[1]} 22%, transparent), transparent 70%)`,
        }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-x-[20%] -inset-y-[30%] -rotate-12 opacity-65 blur-[28px]"
      >
        {colors.map((color, i) => (
          <span
            key={color + i}
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_10%,rgba(8,12,18,0.6)_100%)]"
      />
