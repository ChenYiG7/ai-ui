...
export function ComparisonSlider({
  ...
  return (
    <div
      className={cn(
        'relative isolate aspect-[4/3] w-full overflow-hidden rounded-2xl border border-white/15 bg-[#16191c]',
        className,
      )}
    >
      <div className="absolute inset-0" inert>
        {after}
      </div>
      <div
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-4 top-4 flex justify-between gap-3"
      >
        <span
          className={cn(
            'rounded-full border border-white/15 bg-black/35 px-2.5 py-1 text-[9px] text-white/90 backdrop-blur-md',
            position < 15 && 'invisible',
          )}
        >
          {beforeLabel}
        </span>
      ...
      <div
        ...
        aria-valuetext={`${beforeLabel} ${position}%，${afterLabel} ${100 - position}%`}
        onKeyDown={onKeyDown}
        onPointerDown={(event) => {
      ...
      >
        <div
          aria-hidden
          className="absolute inset-y-0 w-px bg-white/85 shadow-[0_0_12px_#0004]"
          style={{ left: `${position}%` }}
        >
          <div className="absolute top-1/2 grid h-11 w-8 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-white/60 bg-[#ebedeb]/90 shadow-[0_3px_14px_#0005] backdrop-blur-md group-focus-visible/compare:ring-4 group-focus-visible/compare:ring-white/20">
            <span className="flex gap-1">
              <span className="h-3 w-px bg-zinc-500" />
              <span className="h-3 w-px bg-zinc-500" />
            </span>
          </div>
        </div>
