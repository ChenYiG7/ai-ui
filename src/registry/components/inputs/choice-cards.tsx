...
export function ChoiceCards({
  ...
  return (
    <fieldset
      disabled={disabled}
      className={cn('w-full min-w-0 space-y-2', disabled && 'opacity-50', className)}
    >
      <legend className="mb-3 text-[10px] font-medium tracking-[0.12em] text-zinc-400 @md:text-xs">
        {label}
      </legend>
      {options.map((option) => (
        <label
        ...
        >
          ...
          <span className="flex items-center gap-3 rounded-xl border border-white/10 bg-[#141618] px-3.5 py-3 shadow-[inset_0_1px_0_#ffffff05] peer-checked:border-[#c4d4bd]/45 peer-checked:bg-[#1c2420] peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-[#d6e6ce] motion-safe:transition-colors motion-safe:duration-200 @md:gap-4 @md:p-4">
            {option.icon && (
              <span
                aria-hidden
                className="grid size-8 shrink-0 place-items-center rounded-lg border border-white/8 bg-white/3 text-[#c4d4bd]"
              >
                {option.icon}
              </span>
            )}
            <span className="min-w-0 flex-1">
              <span className="flex flex-wrap items-center gap-2">
                ...
                {option.badge && (
                  <span className="rounded border border-[#c4d4bd]/20 bg-[#c4d4bd]/8 px-1.5 py-0.5 text-[8px] text-[#d5e3cd]">
                    {option.badge}
                  </span>
                )}
              </span>
              {option.description && (
                <span className="mt-1 block text-[10px] leading-relaxed text-zinc-400 @md:text-xs">
                  {option.description}
                </span>
              )}
            </span>
            {option.meta && (
              <span className="shrink-0 font-mono text-xs tabular-nums text-zinc-300 @md:text-sm">
                {option.meta}
              </span>
            )}
            <span
