...
export function RotaryDial({
  ...
  const normalize = (next: number) => {
    ...
    return Math.min(
      ...
      Math.max(
        lower,
        Number((lower + Math.round((next - lower) / increment) * increment).toFixed(6)),
      ),
    );
  };
  ...
  return (
    <div className={cn('flex flex-col items-center', disabled && 'opacity-45', className)}>
      ...
      <div
        ...
        className={cn(
          'relative my-2 size-32 touch-none select-none rounded-full outline-none focus-visible:ring-2 focus-visible:ring-[#c4d4bd]/70 focus-visible:ring-offset-4 focus-visible:ring-offset-[#111315] @md:my-3 @md:size-44',
          ...
          {Array.from({ length: 41 }, (_, index) => {
            ...
            return (
              <span
                key={index}
                className={cn(
                  'absolute w-px -translate-x-1/2 -translate-y-1/2 rounded-full',
                  index % 5 === 0 ? 'h-2.5' : 'h-1.5',
                  index / 40 <= ratio ? 'bg-[#d0dfbf] shadow-[0_0_5px_#c4d4bd45]' : 'bg-white/15',
                )}
                style={{
                  left: `${Number((50 + Math.sin(radians) * 46).toFixed(4))}%`,
                  top: `${Number((50 - Math.cos(radians) * 46).toFixed(4))}%`,
                  rotate: `${angle}deg`,
                }}
              />
            );
          ...
          <span className="absolute inset-[14%] rounded-full border border-white/15 bg-[conic-gradient(from_30deg,#555955,#202322_20%,#454944_45%,#1c201e_65%,#62665e_85%,#555955)] p-1 shadow-[0_10px_18px_#0008,inset_0_1px_0_#ffffff30]">
            ...
            <span
              className={cn(
                ...
                !dragging && 'motion-safe:transition-[rotate] motion-safe:duration-150',
              )}
      ...
      <output
        aria-hidden
        className="-mt-1 font-mono text-2xl font-light tabular-nums tracking-tight text-[#e0e7d7] @md:text-3xl"
      >
        {current}
