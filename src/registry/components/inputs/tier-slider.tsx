...
export function TierSlider({
  ...
  return (
    ...
    >
      <style href="tier-slider-keyframes" precedence="medium">
        {KEYFRAMES}
      </style>
      <div className="mb-6 flex items-center gap-2 @md:gap-3.5">
        <span
          aria-hidden
          className="grid size-10 shrink-0 place-items-center rounded-xl bg-white/[0.06] text-zinc-400"
        >
          {icon ?? (
            <svg
              viewBox="0 0 24 24"
        ...
        <div className="flex flex-1 items-baseline justify-center gap-2 whitespace-nowrap">
          <span className="text-lg font-semibold tracking-tight tabular-nums text-white @md:text-2xl">
            {label}
          </span>
          <span
            ref={wordRef}
      ...
      <div className="relative h-[52px]">
        ...
        >
          <div className="absolute inset-x-0 top-1/2 h-11 -translate-y-1/2 rounded-full bg-white/[0.07]">
            <div
              ref={fillRef}
              className="tier-slider-fill absolute inset-y-0 left-0 rounded-full"
              style={{ width: `${startT * 100}%`, ...fillPaint(startT, startT * 430) }}
            >
            ...
            <div
              aria-hidden
              className="pointer-events-none absolute inset-x-0 top-1/2 flex -translate-y-1/2 justify-between px-[34px]"
            >
          ...
          >
            <span
              ...
              style={{
                borderColor: hsl({ h: startColor.h, s: Math.min(startColor.s + 15, 95), l: 62 }),
              }}
            />
          </div>
        </div>
