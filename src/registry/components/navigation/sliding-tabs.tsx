  ...
  return (
    <div className={cn('w-full', className)}>
      <div
        role="tablist"
        aria-label={label}
        className="flex gap-1 overflow-x-auto rounded-xl border border-white/10 bg-[#101314] p-1.5 shadow-[inset_0_2px_5px_#0005]"
      >
        {tabs.map((tab, index) => (
          <button
            key={tab.id}
            ...
            className={cn(
              'relative flex min-h-10 flex-1 items-center justify-center gap-2 whitespace-nowrap rounded-lg px-3 text-[11px] font-medium outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#abc39c] disabled:cursor-not-allowed disabled:opacity-35 @md:px-5 @md:text-xs',
              active.id === tab.id ? 'text-[#233020]' : 'text-zinc-400 hover:text-zinc-200',
            )}
          >
            {active.id === tab.id && (
              <motion.span
