  ...
  return (
    <section aria-label={label} className={cn('w-full max-w-sm', className)}>
      <div className="mb-3 flex items-center justify-between px-1 text-[10px] text-zinc-400">
        <span>{label}</span>
        <span className="font-mono text-zinc-500">{String(pending.length).padStart(2, '0')}</span>
      </div>
      <ol
        aria-live="polite"
        aria-relevant="additions removals"
        className={cn(
          'relative h-34 @md:h-44',
          expanded ? 'overflow-y-auto overscroll-contain pr-1' : 'overflow-hidden',
        )}
      >
        <AnimatePresence initial={false}>
          {visible.map((item, index) => (
            <motion.li
              ...
              animate={{
                top: expanded ? index * 120 : index * 14,
                scale: expanded ? 1 : 1 - index * 0.055,
                opacity: expanded ? 1 : 1 - index * 0.22,
              }}
              exit={{ opacity: 0, scale: reduced ? 1 : 0.97 }}
              transition={
                reduced ? { duration: 0 } : { type: 'spring', stiffness: 360, damping: 32 }
              }
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <p className="truncate text-[11px] font-medium text-zinc-100 @md:text-xs">
                    {item.title}
                  </p>
                  {item.meta && (
                    <span className="shrink-0 text-[8px] text-zinc-500">{item.meta}</span>
                  )}
                </div>
                {item.description && (
                ...
                <button
                  ...
                  className="mt-1 min-h-6 rounded px-0.5 text-[9px] text-zinc-500 hover:text-zinc-200 focus-visible:outline-2 focus-visible:outline-[#c0d9ad]"
                >
                  关闭
                </button>
              </div>
        ...
        {pending.length === 0 && (
          <li
            ref={empty}
            tabIndex={-1}
            className="grid h-28 place-items-center rounded-2xl border border-dashed border-white/10 text-xs text-zinc-500 outline-none"
          >
            {emptyMessage}
          </li>
        )}
