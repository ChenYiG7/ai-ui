...
export function HoldToConfirm({
  ...
  return (
    <div className={cn('w-full max-w-xs', className)}>
      <button
        type="button"
        disabled={disabled || status === 'busy' || status === 'success' || status === 'error'}
        aria-label={label}
        ...
        className={cn(
          ...
          status === 'holding' &&
            'translate-y-1 shadow-[inset_0_1px_0_#fff,0_2px_0_#777e69,0_6px_16px_#0005] motion-reduce:translate-y-0',
      ...
      >
        <span
          ref={fill}
          aria-hidden
          className="absolute inset-0 origin-left bg-[#bacda2]"
          style={{ scale: '0 1' }}
        />
        <span className="relative flex items-center gap-2.5 text-xs font-semibold @md:text-sm">
          {icon && <span aria-hidden>{icon}</span>}
          {message}
        </span>
        <span
          aria-hidden
          className="relative rounded-md border border-black/10 bg-white/25 px-2 py-1 font-mono text-[9px]"
        >
          {(delay / 1000).toFixed(1)} s
        </span>
      </button>
      <p className="mt-5 text-center text-[10px] leading-relaxed text-zinc-500">
        按住鼠标、空格或 Enter，松开即取消
      </p>
