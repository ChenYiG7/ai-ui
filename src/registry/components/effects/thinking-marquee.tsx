...
export function ThinkingMarquee({
  ...
  const options = useRef<PlaybackOptions>({
    text,
    streaming,
    interval,
    loop,
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      root.dataset.state =
        !source && !options.current.streaming
          ? 'idle'
          : done && !options.current.streaming
            ? 'done'
            : 'thinking';
    ...
    const tick = (now: number) => {
      ...
      if (reduced && cursor < source.length) {
        ...
        cursor < source.length &&
        (settings.streaming || now - lastEmission >= Math.max(8, settings.interval))
      ) {
        const fragment = document.createDocumentFragment();
        // 每帧最多插入 48 个节点；按字素切分，避免拆开组合字符。
        for (
          let count = 0;
          count < (settings.streaming ? 48 : 1) && cursor < source.length;
          count++
        ) {
          let chunk = '';
      ...
      const settled =
        cursor === source.length &&
        target - removedWidth - position < 0.5 &&
        (reduced || now - lastEmission >= 620);
      root.dataset.state =
        !source && !settings.streaming
          ? 'idle'
          : settled && !settings.streaming
            ? 'done'
            : 'thinking';
  ...
  return (
    <div
      ref={rootRef}
      role="group"
      aria-label={text || '等待思考内容'}
      data-state="idle"
      className={cn(
        'thinking-marquee w-full min-w-0 text-[15px] font-light leading-[1.85] tracking-[0.024em] text-[oklch(72%_0.01_250)]',
        className,
      )}
    >
      <div aria-hidden className="flex items-center gap-3">
        <div
          ref={viewportRef}
          className="thinking-marquee-viewport relative flex h-[1.85em] min-w-0 flex-1 items-center overflow-hidden"
        >
          <div
            ref={trackRef}
            className="flex w-max shrink-0 items-baseline whitespace-pre will-change-transform"
          />
          {shine && (
            <span className="thinking-marquee-shine pointer-events-none absolute inset-0 bg-[linear-gradient(110deg,transparent_38%,rgba(255,255,255,0.12)_50%,transparent_62%)] bg-[length:200%_100%] bg-no-repeat opacity-0 mix-blend-screen" />
          )}
        </div>
