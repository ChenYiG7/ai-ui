...
export function Dock({
  ...
  const growth =
    Math.min(items.length, Math.ceil(distance / (size + 8))) * Math.max(0, magnification - 1);
  const fittedSize = Math.max(
    16,
    Math.min(
      size,
      (available - 26 - Math.max(0, items.length - 1) * 8) / Math.max(1, items.length + growth),
    ),
  );
  ...
  return (
    <div ref={rootRef} className="flex w-full min-w-0 max-w-lg justify-center py-10">
      <nav
        ...
        onBlur={(e) => {
          if (!e.currentTarget.contains(e.relatedTarget)) mouseX.set(Number.POSITIVE_INFINITY);
        }}
        className={cn(
    ...
    <motion.button
      ...
      onFocus={() => {
        if (!reduce && ref.current) {
          ...
          mouseX.set(bounds.left + bounds.width / 2);
        }
      }}
