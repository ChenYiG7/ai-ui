  ...
  return (
    ...
    >
      <div className="flex h-full w-max gap-3 py-4 @md:gap-5 @md:py-6">
        <div aria-hidden className="w-[calc(14cqw-12px)] shrink-0 @md:w-[calc(16cqw-20px)]" />
        {items.map((item, index) => (
          ...
          >
            <div
              aria-hidden
              className="absolute inset-0 bg-[linear-gradient(145deg,rgba(255,255,255,0.09),transparent_45%,rgba(0,0,0,0.45))]"
            />
            <p className="relative text-[9px] uppercase tracking-[0.28em] text-white/55">
              {item.eyebrow}
            </p>
            <p
              aria-hidden
              className="absolute right-4 top-1 text-7xl font-semibold tracking-tighter text-white/[0.06] @md:text-9xl"
            >
              {String(index + 1).padStart(2, '0')}
            </p>
            <div className="absolute inset-x-5 bottom-5 @md:inset-x-8 @md:bottom-8">
              <h3 className="text-xl font-semibold tracking-[-0.035em] @md:text-3xl">
                {item.title}
              </h3>
              <p className="mt-2 max-w-sm text-xs leading-relaxed text-white/55 @md:text-sm">
                {item.description}
              </p>
            </div>
          </article>
