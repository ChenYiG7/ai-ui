...
export function DecryptText({
  ...
  return (
    <span
      ...
      className={cn(
        'inline-block rounded-sm font-mono tabular-nums outline-none focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-current',
        className,
      )}
    >
      <span aria-hidden>
        {chars.map((char, i) => (
          <span key={i} className="relative inline-grid">
            ...
            <span className="col-start-1 row-start-1 text-center whitespace-pre">
              {displayed[i] ?? char}
            </span>
          </span>
        ))}
