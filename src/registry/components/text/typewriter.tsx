...
export function Typewriter({
  ...
  const chars = Array.from(
    new Intl.Segmenter('zh-CN', { granularity: 'grapheme' }).segment(phrase),
    (part) => part.segment,
  );
  ...
  return (
    <span className={cn('relative inline-grid max-w-full align-baseline', className)}>
      ...
      >
        ...
        <span className="ml-0.5 text-current/70 motion-safe:animate-[typewriter-blink_1.1s_step-end_infinite]">
          {cursor}
        </span>
      </span>
      <span className="sr-only" aria-live="polite" aria-atomic="true">
        {reduced ? phrases[0] : announcement}
      </span>
