...
interface ChartPoint extends MetricPoint {
  x: number;
  y: number;
}
...
export function MetricCard({
  ...
  return (
    <figure
      className={cn(
        'w-full overflow-hidden rounded-2xl border border-white/10 bg-[#141719] p-3 shadow-[inset_0_1px_0_#ffffff08,0_20px_50px_-35px_#000] @xs:p-4 @md:p-6',
        className,
      )}
    >
      <figcaption className="flex items-center justify-between gap-3 text-[10px] text-zinc-400 @md:text-xs">
        ...
        {change && (
          <span
            ...
            style={{
              color: stroke,
              borderColor: `color-mix(in srgb, ${stroke} 22%, transparent)`,
              backgroundColor: `color-mix(in srgb, ${stroke} 6%, transparent)`,
            }}
          >
            {change}
          </span>
      ...
      <p className="mt-1 min-h-3 text-[9px] text-zinc-500 @md:mt-1.5 @md:text-[10px]">
        {selected?.label ?? footnote}
      </p>
      <div className="relative mt-2 h-10 rounded focus-within:outline-2 focus-within:outline-offset-4 focus-within:outline-[#c4d4bd]/60 @xs:mt-3 @xs:h-14 @md:mt-4 @md:h-28">
        ...
        {points.length > 1 && (
          <input
            ...
            onFocus={() => setActive((previous) => previous ?? points.length - 1)}
      ...
      <div className="mt-1 flex justify-between font-mono text-[8px] text-zinc-500 @xs:mt-2 @md:text-[9px]">
        <span>{points[0]?.label ?? '暂无数据'}</span>
        <span>{points.length > 1 ? points[points.length - 1].label : ''}</span>
      </div>
    </figure>
  );
}
