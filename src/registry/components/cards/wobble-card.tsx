...
export function WobbleCard({ children, className }: WobbleCardProps) {
  ...
  const borderRadius = useTransform(() => {
    ...
    return [
      ...
      .map((value) => `${Math.max(14, value)}px`)
      .join(' ');
  });
  ...
  const contentStyle: MotionStyle = {
    x: useSpring(
      useTransform(x, (value) => value * -6),
      spring,
    ),
    y: useSpring(
      useTransform(y, (value) => value * -4),
      spring,
    ),
  };
  ...
  const onMove = (event: PointerEvent<HTMLDivElement>) => {
    if (
      reduce ||
      !event.isPrimary ||
      (event.pointerType === 'touch' && activePointer.current === null)
    )
      return;
  ...
  const onPress = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (
      ...
      event.target.closest("a, button, input, select, textarea, [contenteditable], [role='button']")
    )
      return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <motion.div style={contentStyle} className="relative">
          {children}
        </motion.div>
      </motion.div>
    </div>
