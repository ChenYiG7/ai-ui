...
export function Hero() {
  return (
    <h1 className="text-6xl font-semibold tracking-tight">
      <TextReveal text="复制一份档案,得到一个惊艳的组件。" />
    </h1>
  );
}`,
    preview: (
      <h3 className="max-w-[14ch] text-center text-2xl font-semibold leading-tight tracking-tight text-balance text-white @md:text-4xl @xl:text-5xl">
...
export function Headline() {
  ...
  },
  {
    ...
    designNotes: [
      '高光是一道 110deg 线性渐变:底色 rgba(255,255,255,0.45) 延伸到 35% 处,#ffffff 高光居于 50%、65% 处回到底色;配合 background-clip:text 与透明文字色,渐变只出现在字形内部',
      '背景尺寸 200% 宽,keyframes 把 background-position 从 200% 0 推到 -200% 0,高光因此循环扫过整行文字',
      '默认 3s 一轮、线性匀速、无限循环,speed 可按秒调节;动画类名挂 motion-safe: 前缀,prefers-reduced-motion 时静止为底色渐变、不再扫光',
      '纯 CSS 实现、零动画库依赖:关键帧经 <style href="shiny-text-keyframes" precedence="medium"> 内联(React 19 自动去重提升),无 JS 逐帧开销',
    ],
    deps: [],
...
export function Badge() {
    ...
    preview: (
      <div className="flex w-full max-w-md flex-col items-center gap-6 text-center">
        ...
        <ShinyText
          text="Shine on."
          speed={2.4}
          className="text-4xl font-semibold tracking-tight @md:text-6xl @xl:text-7xl"
        />
      </div>
    ),
  },
  {
    ...
    designNotes: [
      'requestAnimationFrame 驱动,每 30ms 更新一帧,每帧多揭示 0.5 个字符,已揭示的字符立即落定为真实内容',
      '未揭示位置从字符集 !<>-_\\/[]{}=+*^?# 与大写字母、数字中随机取字,空格始终保持不变',
      '每个字符独立 inline-grid:原字符 invisible 占位锁定中西文宽度,乱码字符叠加其上,逐帧换字不推动布局;外层 aria-label 保留原文、内部 aria-hidden',
      '悬停或键盘聚焦重播(tabIndex=0,聚焦环 2px、偏移 4px);animateOnMount=false 时只响应交互,loop 时播完停 2.2s 重播,卸载时清理动画帧与定时器',
      '服务端与首帧都渲染最终文本,挂载后才开始乱码,避免水合不一致;prefers-reduced-motion 时跳过乱码直接显示完整原文',
    ],
    deps: ['motion'],
...
export function Terminal() {
    ...
    preview: (
      <div className="flex w-full max-w-md flex-col items-center gap-4 text-center">
        <DecryptText
          text="ACCESS GRANTED"
          loop
          className="text-3xl font-medium tracking-[0.12em] text-lime-300 @md:text-5xl"
        />
        <DecryptText
          text="悬停或聚焦,重新解码"
          frameDelay={45}
  ...
  },
  {
    ...
    designNotes: [
      '默认每 2.4s 切换一次(interval 可按毫秒调节),AnimatePresence 中新旧词同步交接,不出现空档',
      '新词从下方 65% 位移、透明并带 4px 模糊进入,旧词向上 65% 淡出并模糊,单次 0.45s、cubic-bezier(0.22,1,0.36,1) 缓出',
      '全部词以 invisible 网格占位锁定最大宽度,容器 overflow-hidden 且 whitespace-nowrap,长短词交替不推动周围文字,越界部分被裁掉',
      'sr-only 节点以 aria-live=polite 播报当前词;prefers-reduced-motion 时不启动轮换定时器、固定首个词,切换也不带位移与模糊(时长归零)',
    ],
    deps: ['motion'],
...
export function Headline() {
  return (
    ...
    preview: (
      <p className="max-w-md text-center text-2xl font-semibold tracking-tight text-white @md:text-4xl @xl:text-5xl">
        把
        <WordRotate className="mx-2 text-lime-300" words={['灵感', '好设计', '细节', '创造力']} />
  ...
  },
  {
    ...
    designNotes: [
      '每个字符套一层 inline-block overflow-hidden 裁切框,字符从 y 110% 升到 0%,逐字从遮罩下方推上来',
      '字符间隔默认 35ms(stagger 可调、首字符前可加 delay),单字 0.6s、cubic-bezier(0.22,1,0.36,1) 缓出,挂载即播;loop 时按总时长播完停 1.8s 自动重播',
      '拆字 span 化且只动 transform(内层带 will-change-transform),不触发布局重排;空格渲染为不断行空格保持字宽',
      '外层 aria-label 保留完整文本、内部字符 aria-hidden;prefers-reduced-motion 时 initial 直接为 visible,跳过动画显示成品',
    ],
    deps: ['motion'],
...
export function Hero() {
  return (
    ...
    preview: (
      <h3 className="max-w-md text-center text-2xl font-semibold tracking-tight text-white @md:text-4xl @xl:text-5xl">
  ...
  },
  {
    ...
    designNotes: [
      'Intl.Segmenter 按词语切分中英文,标点附着前词、空格独立保留;每词 opacity 0→1、scale 1.06→1、blur 16px→0,从失焦光斑中对焦成形',
      '单词 0.55s、cubic-bezier(0.22,1,0.36,1) 缓出,词间隔默认 120ms(stagger 可调、首词前可加 delay),挂载即播;loop 时按总时长播完停 1.8s 自动重播',
      '拆词 span 化,动画只作用于词级 span 的 opacity/scale/filter,不改变文档布局;词容器 inline-block 且 overflow-wrap:anywhere,超长词允许折行',
      '外层 aria-label 保留完整文本、内部词 aria-hidden;prefers-reduced-motion 时跳过动画直接显示成品',
    ],
    deps: ['motion'],
...
export function SubHeadline() {
  return (
    ...
    preview: (
      <p className="max-w-md text-center text-2xl leading-relaxed text-zinc-300 @md:text-3xl @xl:text-4xl">
