...
# corepack 按 package.json 的 packageManager 字段激活 pnpm
RUN corepack enable
ENV COREPACK_ENABLE_DOWNLOAD_PROMPT=0
ENV NEXT_TELEMETRY_DISABLED=1

# 先装依赖：仅清单文件变动才重装，pnpm store 走构建缓存
# pnpm-workspace.yaml 必须一起 COPY——pnpm 12 的 allowBuilds 白名单在这里，缺了 esbuild 的安装脚本不执行，tsx 起不来
...
# nginx 站点配置模板与容器入口脚本
COPY docker/nginx.conf /etc/nginx/http.d/ai-ui.conf.template
